...
export default function VokalSceneCard({
  ...
  return (
    <View style={{ width: cardWidth, height: cardHeight }} className="bg-white">
      <View className="relative h-full overflow-hidden rounded-3xl shadow-lg">
        <ImageBackground
          source={{ uri: imageUrl }}
          className="h-full w-full justify-end"
          resizeMode="cover">
          <LinearGradient
            colors={['transparent', scene.isLocked ? 'rgba(0,0,0,0.9)' : 'rgba(0,0,0,0.8)']}
            className={`absolute bottom-0 w-full ${scene.isLocked ? 'z-10' : ''}`}
            style={{
              height: scene.isLocked ? cardHeight : cardHeight * 0.4,
            }}
          />
